...
#	--vm-name "$VM_NAME_1" -n "PROVIDE_DATA_DISK_NAME_HERE"

#az vm disk detach --subscription "$SUBSCRIPTION_ID" -g "$RG_NAME_DEPLOY" --verbose \
#	--vm-name "$VM_NAME_2" -n "PROVIDE_DATA_DISK_NAME_HERE"
...
az vm update --subscription "$SUBSCRIPTION_ID" -g "$RG_NAME_DEPLOY" --verbose \
	-n "$VM_NAME_1" --os-disk "$newVmOsDiskIdVm1"

az vm update --subscription "$SUBSCRIPTION_ID" -g "$RG_NAME_DEPLOY" --verbose \
	-n "$VM_NAME_2" --os-disk "$newVmOsDiskIdVm2"
