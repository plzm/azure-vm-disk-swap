...
echo "Create Image Definition for src VM - 1"
az sig image-definition create --subscription "$SUBSCRIPTION_ID" -g "$RG_NAME_SIG" -l "$LOCATION" --verbose \
	-r "$SIG_NAME" --gallery-image-definition "$VM_IMG_DEF_NAME_V2" --os-type "$VM_OS_TYPE" \
	--publisher "$OS_PUBLISHER_2" --offer "$OS_OFFER_2" --sku "$OS_SKU_2" \
	--hyper-v-generation "$HYPER_V_GENERATION" --os-state "$OS_STATE"

echo "Create Image Definition for src VM - 2"
az sig image-definition create --subscription "$SUBSCRIPTION_ID" -g "$RG_NAME_SIG" -l "$LOCATION" --verbose \
	-r "$SIG_NAME" --gallery-image-definition "$VM_IMG_DEF_NAME_V3" --os-type "$VM_OS_TYPE" \
